...
class Article:
    ...
    def publication_date(soup):
        ...
        try:
            ...
            for tag in soup.find_all('head'):
                publication_date= tag.find('meta',attrs={'name':'citation_cover_date'})
                if publication_date != None:
                    publication_date=publication_date['content']
                    publication_date=publication_date.replace('/','-')
                    return publication_date
    ...
    def abstract(soup):
        ...
        try:
            for tag in soup.find_all('body'):
                para=tag.find('p',attrs={'class':'Para'})
                if para != None:
                    abstract=para.get_text()
                    return abstract
                else:
                    for tag in soup.find_all('head'):
    ...
    def keywords(soup):
        ...
        try:
            keywords=[]
            for tag in soup.find_all('div', attrs={'class':'KeywordGroup'}):
...
if __name__ == "__main__":
    ...
    """ [test url: "https://link.springer.com/article/10.1007/s10765-013-1554-4" ] """
